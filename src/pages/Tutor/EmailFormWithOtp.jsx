...
export default function EmailFormWithOtp() {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      setError(
        ...
        'Unable to send OTP. Email may not exist or already registered.'
      );
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (otp.length < 6) {
      setError('OTP code must be at least 6 characters');
      return;
    }
  ...
  const handleResendOtp = async (e) => {
    ...
    try {
      ...
      setSuccessMessage('A new OTP code has been sent! Please check your email');
      setOtp('');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#f7f2ed] flex flex-col">
      ...
      <div className="w-full bg-white py-6 px-8 flex justify-between items-center shadow-sm">
        ...
        <h1 className="text-3xl font-bold text-gray-800">Tutor Sign Up</h1>
        <button
          onClick={() => navigate('/')}
          className="text-3xl text-gray-600 hover:text-gray-800 w-10 h-10 flex items-center justify-center"
          title="Close"
      ...
      <div className="flex-1 flex justify-center items-center px-4">
        <div className="bg-white rounded-xl shadow-md p-8 max-w-4xl w-full flex flex-col md:flex-row gap-12">
          ...
          <div className="md:w-1/2 flex flex-col items-center justify-center">
            ...
            />
            <h2 className="text-2xl font-bold mb-3">Join as a Tutor</h2>
            <p className="text-gray-600 text-center text-lg">
              Share your knowledge and start earning by teaching students online
            </p>
          </div>
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-4 text-center text-2xl tracking-widest font-bold border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#03ccba] focus:border-transparent outline-none transition-all uppercase"
                    required
                    maxLength="6"
                  />
                  <p className="text-xs text-gray-500 mt-2">Enter the 6-character code from your email</p>
                </div>
